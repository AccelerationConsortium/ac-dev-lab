...
def check_workspace_limits(position):
    ...
    if not (WORKSPACE_LIMITS['x_min'] <= x <= WORKSPACE_LIMITS['x_max']):
        print(f"⚠️  X position {x} outside limits [{WORKSPACE_LIMITS['x_min']}, {WORKSPACE_LIMITS['x_max']}]")
        return False
    
    if not (WORKSPACE_LIMITS['y_min'] <= y <= WORKSPACE_LIMITS['y_max']):
        print(f"⚠️  Y position {y} outside limits [{WORKSPACE_LIMITS['y_min']}, {WORKSPACE_LIMITS['y_max']}]")
        return False
    
    if not (WORKSPACE_LIMITS['z_min'] <= z <= WORKSPACE_LIMITS['z_max']):
        print(f"⚠️  Z position {z} outside limits [{WORKSPACE_LIMITS['z_min']}, {WORKSPACE_LIMITS['z_max']}]")
        return False
