...
# Configuration
DURATION_SECONDS = 10 * 60  # 10 minutes minimum
...
def signal_handler(signum, frame):
    """Handle user interruption (Ctrl+C) to gracefully stop recording"""
    global process
...
print(f"Recording display {DISPLAY} for {DURATION_SECONDS // 60} minutes...")
...
try:
    ...
    print("\nRecording interrupted by user - saving video...")
    success = True  # Mark as success so we check the output file
    
finally:
    # Ensure we always try to save/check the video regardless of how we exit
    if process:
        try:
            process.terminate()
    ...
    if success and output_file and os.path.exists(output_file):
        try:
            ...
            print(f"Video file exists but couldn't read size: {e}")
    elif success and os.path.exists(OUTPUT_DIR):
        # Check for screenshot sequence
        screenshots = [f for f in os.listdir(OUTPUT_DIR) if f.startswith('frame_')]
        if screenshots:
