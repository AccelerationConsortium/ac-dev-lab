...
Copy the following code into the file (right click to paste), save it via `Ctrl+O` and `Enter` and exit via `Ctrl+X`. NOTE: This assumes that your username is `ac` rather than the default username for <REDACTED> pi.
